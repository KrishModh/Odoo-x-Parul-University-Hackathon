...
export default function Dashboard() {
  ...
  const dashboardStats = [
    { label: 'Active trips', value: String(savedTrips.length || 4).padStart(2, '0'), trend: savedTrips.length ? 'Synced from PostgreSQL' : '+2 this month' },
    ...stats.slice(1)
  ];
  const visibleTrips = useMemo(() => {
    if (!savedTrips.length) return trips;
    ...
    return savedTrips.map((trip) => ({
      ...
      budget: `INR ${Number(trip.estimated_budget).toLocaleString()}`,
      status: trip.visibility === 'public' ? 'Public' : 'Private',
      progress: 55,
      image: trip.cover_image || trips[0].image
    }));
  }, [savedTrips]);
        ...
        <motion.section className="dashboard-hero-panel" initial="hidden" animate="visible" variants={fadeUp}>
          <div className="hero-panel-copy">
            ...
            <div className="hero-panel-actions">
              <button className="primary-button" type="button" onClick={() => navigate(savedTrips[0] ? `/trip/${savedTrips[0].id}/itinerary` : '/create-trip')}><FiEdit3 /> Continue planning</button>
            ...
            <section className="dashboard-section">
              ...
              <div className="quick-action-grid">
                {quickActions.map((action) => (
                  <button type="button" key={action.label} onClick={() => action.path && navigate(action.path)}>
                    {action.icon}
